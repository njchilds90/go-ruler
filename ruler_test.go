...
func TestEquals(t *testing.T) {
	tests := []struct {
		...
		{"match string", ruler.Equals("status", "active"), ruler.FactMap{"status": "active"}, true},
		{"no match string", ruler.Equals("status", "active"), ruler.FactMap{"status": "inactive"}, false},
		{"missing field", ruler.Equals("status", "active"), ruler.FactMap{}, false},
		{"match int", ruler.Equals("code", 42), ruler.FactMap{"code": 42}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			e.MustAddRule(ruler.Rule{
				Name:       "r",
				Op:         ruler.OpAnd,
				Conditions: []ruler.Condition{tt.cond},
			})
...
func TestMatches(t *testing.T) {
	tests := []struct {
		...
		{"uuid prefix", `^[0-9a-f]{8}-[0-9a-f]{4}`, "550e8400-e29b-41d4", true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			e := ruler.NewEngine()
...
func TestEvaluateAll_ReturnsAll(t *testing.T) {
	...
	e.MustAddRule(ruler.Rule{
		Name:       "no-match",
		Op:         ruler.OpAnd,
		Conditions: []ruler.Condition{ruler.Equals("x", 2)},
	})
...
func TestResult_Fields(t *testing.T) {
	...
	e.MustAddRule(ruler.Rule{
		Name:        "tagged-rule",
		Description: "A rule with tags and metadata",
		Score:       42,
		Op:          ruler.OpAnd,
		Tags:        []string{"risk", "scoring"},
		Metadata:    map[string]any{"owner": "fraud-team"},
		Conditions:  []ruler.Condition{ruler.Equals("flag", true)},
	})

	res, err := e.EvaluateMatching(context.Background(), ruler.FactMap{"flag": true})
	if err != nil {
		t.Fatal(err)
	}
	if len(res) != 1 {
	...
	r := res[0]
	if r.RuleDescription != "A rule with tags and metadata" {
		t.Errorf("unexpected description: %q", r.RuleDescription)
	}
	if r.Score != 42 {
		t.Errorf("expected score 42, got %v", r.Score)
	}
	if len(r.Tags) != 2 || r.Tags[0] != "risk" {
		t.Errorf("unexpected tags: %v", r.Tags)
	}
	if r.Metadata["owner"] != "fraud-team" {
...
func TestNotEquals(t *testing.T) {
	...
	e.MustAddRule(ruler.Rule{
		Name:       "not-banned",
		Op:         ruler.OpAnd,
		Conditions: []ruler.Condition{ruler.NotEquals("status", "banned")},
	})

	t.Run("passes when different", func(t *testing.T) {
		res, _ := e.EvaluateMatching(context.Background(), ruler.FactMap{"status": "active"})
		if len(res) != 1 {
			t.Error("expected match")
		}
	})
	t.Run("fails when equal", func(t *testing.T) {
		res, _ := e.EvaluateMatching(context.Background(), ruler.FactMap{"status": "banned"})
		if len(res) != 0 {
			t.Error("expected no match")
		}
	})
...
// --- Score is zero when unmatched ---

func TestScore_ZeroWhenUnmatched(t *testing.T) {
	e := ruler.NewEngine()
	e.MustAddRule(ruler.Rule{
		Name:       "scored",
		Score:      99,
	...
	all, err := e.EvaluateAll(context.Background(), ruler.FactMap{"x": 99})
	if err != nil {
		t.Fatal(err)
	}
	if len(all) != 1 {
	...
	if all[0].Score != 0 {
		t.Errorf("expected score 0 for unmatched rule, got %v", all[0].Score)
	}
}
...
func TestRuleCount(t *testing.T) {
	...
	e.MustAddRule(ruler.Rule{Name: "r1", Op: ruler.OpAnd, Conditions: []ruler.Condition{ruler.Exists("x")}})
	e.MustAddRule(ruler.Rule{Name: "r2", Op: ruler.OpAnd, Conditions: []ruler.Condition{ruler.Exists("x")}})
	if e.RuleCount() != 2 {
		t.Errorf("expected 2, got %d", e.RuleCount())
	}
}
